...
public class CSVLineToTemporalEdge extends CSVLineToTemporalElement<TemporalEdge> {
  ...
  public TemporalEdge map(String csvLine) throws Exception {
    ...
    File file = new File("/shared_folder/gradoop-shared/logger_output/EdgeLog.txt");

    try {
      FileOutputStream fileOutputStream = new FileOutputStream("/shared_folder/gradoop-shared/logger_output/EdgeLog.txt",true);
      PrintStream printStream = new PrintStream(fileOutputStream);
      System.setOut(printStream);

      System.out.println("TaskSlot: "+ getRuntimeContext().getIndexOfThisSubtask() + ", GraphId: " + tokens[1] + ", EdgeId: " + GradoopId.fromString(tokens[0]) + " \n");
